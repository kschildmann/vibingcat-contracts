...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    function safeVibingCatTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > vibingcatBal) {
            transferSuccess = vibingcat.transfer(_to, vibingcatBal);
        } else {
            transferSuccess = vibingcat.transfer(_to, _amount);
        }
        require(transferSuccess, "safeVibingCatTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _vibingcatPerBlock) public onlyOwner {
        massUpdatePools();
        vibingcatPerBlock = _vibingcatPerBlock;
        emit UpdateEmissionRate(msg.sender, _vibingcatPerBlock);
    }
}
